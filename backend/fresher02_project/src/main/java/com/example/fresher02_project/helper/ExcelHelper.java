...
public class ExcelHelper {
    ...
    public static ByteArrayInputStream customerToExcel(List<Customer> customers) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            String[] headers = {"ID", "Name", "Phone", "Email", "Address", "Id User create", "Time Create"};
            ...
            for (Customer customer : customers) {
                ...
                row.createCell(5).setCellValue(customer.getCreateByIdUser());
    ...
    public static ByteArrayInputStream userToExcel(List<User> users) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            // Header
            Row headerRow = sheet.createRow(0);
            String[] headers = {"ID", "User Name", "Name", "Phone", "Email", "Address", "Id Admin create", "Time Create"};
            setHeader(headers, headerRow);
    ...
    public static ByteArrayInputStream historyCareToExcel(List<HistoryCare> historyCares) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            // Header
            Row headerRow = sheet.createRow(0);
            String[] headers = {"ID", "ID Customer", "User Name","Start Time","End Time", "Concern Level", "Note", "Potential Level", "Next Action", "Menthod", "Purpose", "Product Name"};
            for (int col = 0; col < headers.length; col++) {
                Cell cell = headerRow.createCell(col);
                cell.setCellValue(headers[col]);
            }
            ...
            for (HistoryCare historyCare : historyCares) {
                ...
                row.createCell(2).setCellValue(historyCare.getUserName());
    ...
    public static ByteArrayInputStream transactionToExcel(List<Transaction> transactions) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            // Header
            Row headerRow = sheet.createRow(0);
            String[] headers = {"ID", "ID Customer", "ID User", "Time Transaction", "ID Product", "Status", "Note"};
            setHeader(headers, headerRow);
            ...
            for (Transaction transaction : transactions) {
                ...
                row.createCell(2).setCellValue(transaction.getIdUser());
